...
ACTION cronjob::insertjob(name owner, name id, name actionname, uint64_t firstRunUTC, uint64_t repeatInS)
{
  ...
  internal_use_do_not_use::eosio_assert(jobIt == cronjobs.end(), "Delete old job first");
  ...
  auto tnow = current_time_point().sec_since_epoch();
  internal_use_do_not_use::eosio_assert(tnow < firstRunUTC, "First run must be in future");
...
ACTION cronjob::runandsched(name owner, name id, uint64_t nRecure)
{
  ...
  auto jobIt = cronjobs.find(id.value);
  internal_use_do_not_use::eosio_assert(jobIt != cronjobs.end(), "Cannot find job");
...
void cronjob::updateLastRun(cronjobs_t& cronjobs, name id)
{
  ...
  cronjobs.modify(jobIt, _self, [&](auto& obj) {
      obj.lastRun = current_time_point().sec_since_epoch();
    });
}

ACTION cronjob::rmalljobs(name owner)
{
  internal_use_do_not_use::eosio_assert(has_auth(owner) or has_auth(_self), "auth from contract or caller needed");
  cronjobs_t cronjobs(_self, owner.value);
  unsigned int i = 0;
  bool left = false;
...
ACTION cronjob::rmjob(name owner, name id)
{
  ...
  internal_use_do_not_use::eosio_assert(it != cronjobs.end(), "Job already deleted?");
  uint128_t txid = (((uint128_t) it->cronjob.id.value) << 32) + it->cronjob.acc.value;
  cancel_deferred(txid);
  cronjobs.erase(it);
}
